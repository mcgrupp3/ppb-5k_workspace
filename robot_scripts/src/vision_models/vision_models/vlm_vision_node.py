...
"""
PPB-5K VLM Vision Node
Runs on Pi 5 (Docker, ROS2 Jazzy)

Subscribes:  /vlm/query   std_msgs/String  JSON {"object_name": "...", "description": "..."}
Publishes:   /vlm/result  std_msgs/String  JSON {"present": bool, "yes_no": "yes"|"no", "raw_answer": "..."}
"""
...
def parse_yes_no(answer: str) -> Tuple[Optional[bool], str]:
    ...
    text  = answer.strip()
    lower = text.lower()
    first = re.sub(r"^[\s\"'`\-]+|[\s\"'`]+$", "", text.splitlines()[0].strip().lower())
    ...
    if yes_m and no_m:
        return (yes_m.start() < no_m.start()), 'first keyword wins'
...
class VlmVisionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('zmq_host',       'localhost')
        self.declare_parameter('zmq_port',       5555)
        self.declare_parameter('zmq_timeout_ms', 8000)
        ...
        self.get_logger().info(
            f'VLM vision node ready — ZMQ tcp://{self._zmq_host}:{self._zmq_port}'
        )
    ...
    def _reset_socket(self) -> None:
        try:
            ...
            pass
        self._socket = self._make_socket()
        self.get_logger().warn('ZMQ socket recreated.')
    ...
    def _on_query(self, msg: String) -> None:
        ...
        if not object_name:
            self.get_logger().error('Missing object_name — ignoring.')
            return
        ...
        try:
            self._socket.send_string(json.dumps({'command': 'query', 'prompt': prompt}))
            raw = self._socket.recv_string()
        except zmq.Again:
            self.get_logger().error('ZMQ timeout.')
            self._reset_socket()
            self._publish_result(present=False, yes_no='no', raw_answer='timeout')
            return
        except zmq.ZMQError as e:
        ...
        if resp.get('status') != 'ok':
            self.get_logger().error(f'Server error: {resp}')
            self._publish_result(present=False, yes_no='no', raw_answer=str(resp))
            return

        answer = resp.get('answer', '').strip()
        parsed, reason = parse_yes_no(answer)

        if parsed is None:
            self.get_logger().warn(f'Ambiguous answer: "{answer}" ({reason}) — treating as no.')
            parsed = False

        yes_no  = 'yes' if parsed else 'no'
        self.get_logger().info(f'Parsed: {yes_no} ({reason})')
        self._publish_result(present=parsed, yes_no=yes_no, raw_answer=answer)

    def _publish_result(self, *, present: bool, yes_no: str, raw_answer: str) -> None:
        payload = {'present': present, 'yes_no': yes_no, 'raw_answer': raw_answer}
        msg = String()
        msg.data = json.dumps(payload)
        self._result_pub.publish(msg)
        self.get_logger().info(f'Published /vlm/result: {payload}')
